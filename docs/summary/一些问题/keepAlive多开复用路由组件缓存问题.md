...
# keepAlive多开复用路由组件缓存问题

## 情景
<a-divider />

如下图所示，有一个列表页，点击编辑按钮进入对应元素的编辑页，对编辑页进行keepalive缓存

<a-image src="https://cdn.jsdelivr.net/gh/zhihao2030/note-img@main/20230117095751.png" />
...
此处仅仅为了复现问题，没做过多的处理，正常情况我一般会watch路由变化，把需要缓存的页面加入

keepAliveList，在关闭页面时移出keepAliveList

::: tip 分析：

我们都知道，keepAlive的includes接收的是组件的name，但我们在以上多开编辑页的情况下，所以编辑页都是一样的name，我们来看看按照常规做法会出现什么问题
:::
## 问题
<a-divider />

<a-image src="https://cdn.jsdelivr.net/gh/zhihao2030/note-img@main/63c6083aa2242_63c6083ac1ad3.gif" />

当我们进行关闭tab，并清除缓存时,编辑页都是一样的name,造成所有编辑页数据丢失
...
<a-image src="https://cdn.jsdelivr.net/gh/zhihao2030/note-img@main/63c60a10e686c_63c60a129ae07.gif" />

可以看见，由于是复用同一组件，所以组件名是相同的，而keepalive又是根据组件名字进行缓存，所以在清除缓存的时候，所有的编辑页缓存都会被清除
...
2. 修改keepalive源码，更改配置方式，让每一个页面在includes中有唯一对应的值，清除时指定清除
...
<a-image src="https://cdn.jsdelivr.net/gh/zhihao2030/note-img@main/20230117104822.png" />

### includeKey, excludeKey 
<a-divider />

增加自定义配置includeKey，与includes不同，includeKey是以页面所在的fullpPath为key进行匹配
...
```ts
import { default as MyKeepAlive } from '@/views/sys/myKeepAlive';
...
<a-image src="https://cdn.jsdelivr.net/gh/zhihao2030/note-img@main/63c628d90b3bb_63c628d92a503.gif" />
