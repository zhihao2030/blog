<template>
  <div class="meta-wrapper">
    ...
    <div class="meta-item">
      ...
      <span class="meta-content">{{ formatDate(article.date) }}</span>
    </div>
    ...
    <div class="meta-item">
      ...
      <span class="meta-content">
        <span v-for="(tag, index) in tags" :key="index">
          <a href="javascript:void(0);" @click="goToLink('/tags', 'tag', tag)" target="_self" :title="tag">{{ tag }}</a>
...
const data = reactive({
  ...
  authorLink: props.article?.authorLink ?? theme.value.authorInfo.authorLink ?? '#',
